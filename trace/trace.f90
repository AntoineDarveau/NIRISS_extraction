...
! Inputs:
!
! - naxes gives the number of columns, naxes[1], and lines,naxes[2]
! - Image is the input image (naxes[1] x naxes[2])
! - bpix is the bad pixel map (not yet well implemented)
! - nline?
...
use precision
implicit none
integer, parameter :: nKsize = 64 ! Size of Kernel for import
...
! Check ncutpsf value is valid ---------

if(ncutpsf.gt.nKsize)then
...
! zero out residuals to avoid problems.
   ...
   enddo
enddo ! end loop over expected number of traces
